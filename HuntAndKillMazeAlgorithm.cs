...
public class HuntAndKillMazeAlgorithm : MazeAlgorithm {
	...
	private void HuntAndKill() {
		mazeCells [currentRow, currentColumn].visited = 1;
	...
	private void Hunt() {
		...
		for (int r = 0; r < mazeRows; r++) {
			for (int c = 0; c < mazeColumns; c++) {
				if (mazeCells [r, c].visited == 0 && CellHasAnAdjacentVisitedCell(r,c)) {
	...
	private bool RouteStillAvailable(int row, int column) {
		...
		if (row > 0 && mazeCells[row-1,column].visited == 0) {
			availableRoutes++;
		}

		if (row < mazeRows - 1 && mazeCells [row + 1, column].visited == 0) {
			availableRoutes++;
		}

		if (column > 0 && mazeCells[row,column-1].visited == 0) {
			availableRoutes++;
		}

		if (column < mazeColumns-1 && mazeCells[row,column+1].visited == 0) {
			availableRoutes++;
		}
	...
	private bool CellIsAvailable(int row, int column) {
		if (row >= 0 && row < mazeRows && column >= 0 && column < mazeColumns && mazeCells [row, column].visited == 0) {
			return true;
		} else {
			return false;
		}
	...
	private bool CellHasAnAdjacentVisitedCell(int row, int column) {
		...
		if (row > 0 && mazeCells [row - 1, column].visited == 1) {
			visitedCells++;
		}

		// Look one row down (south) if we're the second-to-last row (or less)
		if (row < (mazeRows-2) && mazeCells [row + 1, column].visited == 1) {
			visitedCells++;
		}

		// Look one row left (west) if we're column 1 or greater
		if (column > 0 && mazeCells [row, column - 1].visited == 1) {
			visitedCells++;
		}

		// Look one row right (east) if we're the second-to-last column (or less)
		if (column < (mazeColumns-2) && mazeCells [row, column + 1].visited == 1) {
			visitedCells++;
		}
